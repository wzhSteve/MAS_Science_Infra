# MAS / ARPO / APPO 多次采样与 Reward 对照说明


| 项   | 内容                                                                           |
| --- | ---------------------------------------------------------------------------- |
| 日期  | 2026-09-14                                                                   |
| 范围  | `MAS_Science_Infra`（含 tir_agent 接入层）、官方 `APPO-main/ARPO`、官方 `APPO-main/APPO` |
...
> **可恢复前缀（token 列表或 messages Snapshot）+ 继续采样**
...
| **声明式屏障（barrier）**      | 图/配置规定「允许在哪些节点 fork」，真正选点由算法做 | MAS `SamplePolicy` + `sample_barrier` 边 |
...
ExecutionService.run / fork(BranchPoint)     ← 只跑图，不算 reward
    │
    ├─ Archive.snapshot(messages[, token_prefix])
    │
    ▼
Collector / LitTirAgent
...
- `Snapshot.token_ids` / `EventKind.ON_TOKEN` / UI `on_token` 候选与 `resume_mode=token_prefix` 已落地；无 token 引擎时 Daemon **降级 messages** 并打 `resume_mode_downgraded`
- 真正的 BS = Z(H)·Z(Ω) 仍宜放在 RL adapter（需要 current/old logprob），经同一 `BranchPoint` 回灌
- **未**把选点塞进 LangGraph 核心（保持接口简单）
- Runner ready-batch：见 [BRANCH_SITE_DESIGN.md](BRANCH_SITE_DESIGN.md) / `ActiveSetScheduler`（伪 batch LLM + 异步 tool；非 vLLM Worker 共置）
...
| 维度          | 官方 ARPO                 | 官方 APPO                   | MAS（tir_agent）                               |
| ----------- | ----------------------- | ------------------------- | -------------------------------------------- |
| 分叉粒度        | tool 轮次后                | **token** procedure point | 对话 Snapshot（tool/agent 屏障）；可扩展 token_prefix  |
...
| Acc>0 且 `n_search>0` 且 `n_python>0`       | **Acc + multi_tool_bonus(默认 0.1)** |
...
| 算法                   | Advantage 行为                                                    |
| -------------------- | --------------------------------------------------------------- |
| **GRPO**             | 同题 N 条轨迹：A_i = (R_i - \mathrm{mean}_R)/\mathrm{std}_R（或仅减均值）    |
| **官方 ARPO**          | 同 GRPO；分支与全局轨迹**一视同仁**进组                                        |
| **官方 APPO**          | init / branch **分组**；branch 提供对比；M 缩放回写 init 前缀；branch 不进 actor |
| **MAS ARPO**         | 同 GRPO（完整轨迹）                                                    |
| **MAS APPO Phase A** | branch 行 advantage×0；init 可按 branch_score 做 M 缩放                |
...
2. 需要 UI 声明采样、Archive/Harness 回放、多 Agent 边类型
3. 保持 **一个分叉入口**（`BranchPoint`），避免 RL 私自再开一套 resume 协议
...
| `workflow/archive.py`              | snapshot / restore / token_prefix           |
