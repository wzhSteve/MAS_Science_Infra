"""Resolve effective inference configuration without exposing credentials."""
...
class EffectiveLlmConfig:
    ...
    def issues(self, *, require_model: bool = True) -> list[dict[str, str]]:
        ...
        if self.kind == "rl_endpoint":
            ...
            issues.append(
                {"code": "invalid_kind", "message": "请选择 API 或本地模型模式。"}
            )
        ...
        if any(char in self.api_key for char in ("\r", "\n", "\x00")):
            issues.append(
                {"code": "invalid_credential", "message": "API Key 包含无效控制字符，请重新配置。"}
            )
        return issues
    ...
    *,
    ...
    if resource is not None:
        ...
        for field_name, value in (
            ...
            if value is not None and value.strip().rstrip("/") != getattr(
                effective, field_name
            ).rstrip("/"):
                raise ResourceError(
                    "当前实验使用模型资源，请修改资源或解除绑定，不能覆盖资源配置。",
                    409,
                )
        return effective
...
def resource_llm_config(
    resource: Any, *, api_key: Optional[str] = None
) -> EffectiveLlmConfig:
    ...
    return EffectiveLlmConfig(
        kind=config.get("kind", "api"),
        model=config["model"],
        base_url=config["base_url"],
        api_key=key,
        credential_source=source,
        resource_id=data["id"],
        resource_revision=data["revision"],
