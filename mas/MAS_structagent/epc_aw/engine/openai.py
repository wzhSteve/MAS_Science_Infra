...
# Never override vars already set by the RL runner (VERL endpoint injection).
...
def _looks_openai_compatible(model_string: str) -> bool:
    """True for local/path/Qwen-style OpenAI-compatible chat servers (vLLM / VERL)."""
    if not model_string:
        return False
    lowered = model_string.lower()
    if model_string.startswith("/") or model_string.startswith("./"):
        return True
    markers = ("qwen", "llama", "mistral", "deepseek", "yi-", "phi-", "gemma", "vllm")
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        """
        :param model_string: model id or local path; wins over MODEL_Name when provided.
        :param base_url: OpenAI-compatible API base; wins over OPENAI_API_BASE_URL when provided.
        :param api_key: optional; falls back to OPENAI_API_KEY or \"dummy\".
        :param openai_compatible: force chat.completions path for non-gpt local servers.
        """
        from .runtime_override import get_llm_runtime_override

        # 1) RL runtime override (VERL endpoint) beats .env and kwargs.
        # 2) Explicit kwargs beat ambient env.
        # 3) Env / dotenv last.
        rt = get_llm_runtime_override()
        self.model_string = (
            (rt.get("model") if rt else None)
            or model_string
        ...
        self.base_url = (
            (rt.get("base_url") if rt else None)
            or base_url
        ...
        self.api_key = (
            (rt.get("api_key") if rt else None)
            or api_key
        ...
        self.SERVER_MODEL = [
            m.strip() for m in (os.getenv("SERVER_MODEL") or "").split(",") if m.strip()
        ]
        if openai_compatible is None:
            if rt and "openai_compatible" in rt:
                openai_compatible = bool(rt["openai_compatible"])
            else:
                env_flag = (os.getenv("MAS_OPENAI_COMPATIBLE") or os.getenv("CD_TAA_OPENAI_COMPATIBLE") or "").strip().lower()
                if env_flag in ("1", "true", "yes", "on"):
                    openai_compatible = True
                else:
                    openai_compatible = (
                        self.model_string in self.SERVER_MODEL
                        or _looks_openai_compatible(self.model_string)
                    )
        ...
        # Never inherit HTTP_PROXY for LLM calls — VERL/LiteLLM/vLLM are local;
        # Clash would return 502. Search tools pass proxies= explicitly.
        import httpx

        self.client = OpenAI(
            api_key=self.api_key,
            base_url=self.base_url,
            http_client=httpx.Client(trust_env=False, timeout=httpx.Timeout(120.0, connect=30.0)),
        )
    ...
    def generate(self, content: Union[str, List[Union[str, bytes]]], system_prompt=None, **kwargs):
        try:
            ...
            msg = str(e)
            # Fail-fast defense-in-depth: a 429 carrying "Invalid schema for
            # response_format" is a PERMANENT proxy rejection (not a transient
            # rate limit). The OpenAI SDK still internally retries 429s before
            # surfacing, so by the time we get here the call already cost some
            # wall time — but returning an error dict (instead of relying on
            # tenacity) lets the caller's single-pass fallback trigger
            # immediately, and the explicit hint makes a future schema bug
            # obvious. The root cure is fixing the response_format schema.
            if "Invalid schema for response_format" in msg:
                print(
                    f"⚠️ Permanent response_format schema rejection (not a rate limit): {msg}\n"
                    f"   → check the pydantic model's Union fields; bare list/dict generate "
                    f"anyOf branches without a `type` key. Returning error so caller falls back."
                )
                return {
                    "error": "response_format_schema_invalid",
                    "message": msg,
    ...
    def _generate_text(
        self, prompt, system_prompt=None, temperature=0, max_tokens=16384, top_p=0.99, n=1, response_format=None
    ):
        sys_prompt_arg = system_prompt if system_prompt else self.system_prompt

        # Cap completion length for local/VERL servers (default CD_TAA 16384 blows
        # max_model_len = max_prompt + max_response, e.g. fast 4096+1024=5120).
        cap_raw = os.getenv("MAS_MAX_COMPLETION_TOKENS") or os.getenv("CD_TAA_MAX_COMPLETION_TOKENS")
        if cap_raw:
            try:
                max_tokens = min(int(max_tokens), int(cap_raw))
            except ValueError:
                pass
        ...
        # Chat / OpenAI-compatible (vLLM, VERL, Qwen, SERVER_MODEL) without structured outputs
        elif (
            self.openai_compatible
            or self.model_string in self.SERVER_MODEL
            or (self.is_chat_model and (not self.support_structured_output or response_format is None))
        ):
            # print(f"Using chat model: {self.model_string} with n: {n}")
            extra_body = None
            if self.openai_compatible or self.model_string in self.SERVER_MODEL:
                # Disable Qwen3 thinking for structured JSON / RL (vLLM uses chat_template_kwargs).
                extra_body = {
            ...
            response = self.client.chat.completions.create(
                model=self.model_string,
                messages=[
                    {"role": "system", "content": sys_prompt_arg + '\n Please strictly do not return or any chain of thought content. Only the final answer is output.'},
                    {"role": "user", "content": prompt},
                ],
                frequency_penalty=0,
                presence_penalty=0,
            ...
            def _strip_think(content: str) -> str:
                ...
                # Unclosed think block (common when max_tokens cuts mid-thought).
                content = re.sub(r"<think>.*", "", content, flags=re.DOTALL | re.IGNORECASE)
                return content.strip()

            if n == 1:
                content = response.choices[0].message.content or ""
                # Prefer reasoning_content strip path for some OpenAI-compat servers.
                response = _strip_think(content)
            else:
                responses = []
                for choice in response.choices:
                    content = choice.message.content or ""
                    responses.append(_strip_think(content))
                response = responses
        ...
        # Reasoning models: Pro models using v1/completions
            ...
            # Fallback: treat as OpenAI-compatible chat (local RL / unknown model ids).
            response = self.client.chat.completions.create(
                model=self.model_string,
                messages=[
                    {"role": "system", "content": sys_prompt_arg},
