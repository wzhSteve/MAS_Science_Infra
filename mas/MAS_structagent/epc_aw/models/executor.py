...
class Executor:
    ...
    def __init__(
        ...
        self.llm_engine = llm_engine or create_llm_engine(
            model_string=llm_engine_name, is_multimodal=False, temperature=temperature
        )
    ...
    def generate_action(
        ...
        response = self.llm_engine(
            prompt, response_format=ToolAction, temperature=self.temperature, n=1
        )
    ...
    def generate_direct_output(self, question: str, state: AgentState) -> str:
        ...
        response = self.llm_engine(
            prompt, response_format=FinalAnswerDraft, temperature=self.temperature, n=1
        )
        try:
