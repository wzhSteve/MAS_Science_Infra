...
class AgentNodeSpec(BaseModel):
    ...
    tools: List[str] = Field(default_factory=list)  # routable tool-agent ids (hub/planner)
...
class RouterSpec(BaseModel):
    """Agent router node (new_framework P1): pick one agent from candidates."""
...
def _normalize_schema03(raw: Dict[str, Any]) -> None:
    ...
    2. Legacy agents without ``kind`` get one inferred from ``role``
       (planner/verifier/hub -> same name; tool-ish roles -> tool; else blank).
    """
    ...
    for a in agents:
        if isinstance(a, dict) and not a.get("kind"):
            ...
            if role in ("planner", "verifier", "hub", "orchestrator"):
                a["kind"] = "hub" if role == "orchestrator" else role
            elif role in ("tool", "executor"):
                a["kind"] = "planner" if role == "executor" else "tool"
            else:
                a["kind"] = "blank"
    if agents:
        raw["agents"] = agents
