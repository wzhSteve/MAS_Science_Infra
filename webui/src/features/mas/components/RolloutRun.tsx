...
import { TrajectoryDetails } from './TrajectoryDetails';
...
export function RolloutConfig({ rollout, pending, executable, readiness, onShowResults }: {
  ...
  return <div className="mas-run-config mas-rollout-config">
    <form onSubmit={event => {
      ...
      <FormField label="任务问题" hint="从 Workflow 入口执行一次，记录 Agent 协作、模型推理与工具交互；不计算奖励。">
      ...
      <FormField label="执行方式" className="mas-rollout-execution">
        <Select value={rollout.execution} disabled={pending}
          onChange={event => rollout.setExecution(event.target.value === 'live' ? 'live' : 'mock')}>
          <option value="mock">模拟执行 · Mock</option><option value="live">真实推理 · Live</option>
        </Select>
      </FormField>
      {rollout.execution === 'live' && <InlineNotice tone="warning">将调用已配置的模型及工具，可能产生费用。就绪检查不代表推理或工具调用已验证。</InlineNotice>}
      ...
      <div className="mas-run-submit">
        <Button type="submit" size="sm" variant="primary" disabled={blocked} loading={rollout.running}>
          <Play size={13} aria-hidden="true" />{rollout.running ? '正在采集 Rollout' : '采集一次 Rollout'}
        </Button>
...
export function RolloutResults({ rollout, workflowChanged, workflow, onLocate, onUseQuestion }: {
  rollout: MasRolloutRun;
  ...
  const changed = historical ? Boolean(context && JSON.stringify(context.workflow) !== JSON.stringify(workflow)) : workflowChanged;
  ...
  return <div className="mas-run-results mas-rollout-results">
    <RolloutHistory history={history} currentRunId={rollout.attempt?.summary?.run_id} />
    ...
    {historical && rollout.running && <InlineNotice tone="info">本页还有一次 Rollout 正在执行；这里显示的是历史记录。</InlineNotice>}
    ...
    {run.error && <InlineNotice tone={run.status === 'unknown' ? 'warning' : 'danger'}>{run.error}</InlineNotice>}
    {!historical && rollout.running && <p className="field-hint">等待同步请求返回；关闭控制台不会取消执行，也不会自动重发。</p>}
    {summary && <>
      {summary.error && <InlineNotice tone="danger">
        {summary.error.stage} / {summary.error.code}：{summary.error.message}
      </InlineNotice>}
      ...
      <p className="field-hint">complete 仅表示本阶段记录契约完整，不等于具备全部训练数据。运行成功或格式通过不代表答案正确；本次未进行奖励评估。</p>
      <div className="mas-rollout-trace">
        <Button size="sm" onClick={() => void readTrajectory()} loading={traceLoading}
          ...
          <FileJson size={14} aria-hidden="true" />{traceError ? '重试读取轨迹' : data ? '刷新轨迹' : '查看轨迹'}
        </Button>
      ...
      {data && <TrajectoryDetails key={data.run.run_id} data={data} experimentId={rollout.experimentId} onLocate={onLocate} />}
      <RolloutExport key={summary.run_id} experimentId={rollout.experimentId} runId={summary.run_id} />
    </>}
    <JsonDetails value={run.workflow} label="查看本次 Workflow 快照" />
    </>}
  </div>;
}

export function RolloutLog({ rollout }: { rollout: MasRolloutRun }) {
  return <div className="mas-run-log"><section>
    <h3>本次单次 Rollout 操作记录</h3>
    <p className="field-hint">来自本页真实保存与请求生命周期，不是模型调用流。详细过程以持久化轨迹为准。</p>
    {!rollout.logs.length ? <p className="mas-console-empty">尚未发起单次 Rollout。</p>
      : <ol>{rollout.logs.map((entry, index) => <li key={index} className={`is-${entry.tone}`}>
        <time>{new Date(entry.at).toLocaleTimeString('zh-CN', { hour12: false })}</time><span>{entry.message}</span>
      </li>)}</ol>}
  </section></div>;
}
