...
function RouterNode({ id, data, selected }: NodeProps<GraphNode>) {
  const cls = [
    ...
    data.issue ? 'has-issue' : '', data.related ? 'is-related' : '', data.canvasMode === 'sampling' ? 'is-sampling-context' : '',
  ].filter(Boolean).join(' ');
  return <div className={cls}>
    ...
    <div className="mas-node__footer">
      <span>Router</span><span>{data.canvasMode !== 'sampling' && data.branchCount ? <span className="mas-node__branch"><GitBranch size={10} />{data.branchCount}</span>
