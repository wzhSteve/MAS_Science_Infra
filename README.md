...
- 默认进入 `#/experiments`：分页浏览实验、按 ID 搜索、填写名称与 ID 创建默认实验，不自动调用模型。
- 工作区地址为 `#/experiments/<id>/workspace`。首页与工作区不设置固定全局侧栏；原 Experiment、LLM、RL、Harness、Monitor 页面从工作区顶部“更多功能”进入，地址为 `#/experiments/<id>/panels/<panel>`。
- 同一实验返回首页、再打开工作区，会保留画布和已访问面板的编辑会话；切换到另一实验时提示处理未保存配置。浏览器刷新 / 关闭后的未保存恢复暂不支持。
- 首页摘要仅加载当前页，不展示推测的最近运行状态。资源、运行详情与训练确认的后续改版见 [`webui/plan/MAS前端产品化实施总览.md`](webui/plan/MAS前端产品化实施总览.md)。

### 画布内实验设置

- “实验设置”在画布右侧打开模型绑定、数据与采样、训练方案、评估与诊断、执行环境；原 LLM / RL / Harness 地址也打开同一份表单，不再各维护一份配置草稿。
- 训练常用参数、完整参数、数据与设备选择共用一份 RL 草稿。各设置显式保存，顶部“保存设计”仍只保存 Workflow。
- “调试”只展开原单题面板，“运行记录”读取原 Rollout 历史，“开始训练”先打开训练配置与已有确认流程；都不会因打开入口自动执行。
- 模型 Key 仍由服务端保存和解析；本地模型启停、训练停止、GPU 推荐、TrainSignal 预填及最近 Collect 诊断继续保留。模型资源可在个人工作空间内跨实验复用；跨用户共享、逐 Agent 模型和统一训练快照不在本阶段实现。

### 模型与数据资源入口

- 首页“模型与数据”或工作区“资源”打开 `#/resources/models` / `#/resources/datasets`。个人模型资源独立于实验，可登记多个推理连接和训练模型来源。
- 资源页负责名称、端点、模型、服务端凭据及训练权重来源的维护；实验设置只选择并保存资源绑定。同一个资源可以被多个实验引用，不是复制多份 Key。
- “默认推理模型”和“训练模型来源”分别选择。资源当前修订用于之后发起的运行，已解析的模型请求和历史记录不改用最新值。资源修订不等于模型权重版本。
- 原实验内配置保持兼容，不自动迁移。新增、编辑和检查模型统一在“管理模型”页面完成；实验设置只保留选择与保存绑定，不提供旧配置编辑或迁移表单。解除引用仍需确认，恢复原实验配置而不复制资源当前凭据。
- 从原实验进入资源库，可选择资源返回原位置作为待保存的绑定；不会自动保存或启动。若旧配置还有未保存修改，先处理原草稿再换绑。
- 可手动预览现有服务端 parquet 任务文件（前 1–20 条）。预览不上传、不注册、不保存训练路径，也不调用模型。
- 数据入口仍按实验展示训练 / 验证路径；数据导入方式、平台共享、逐 Agent 模型暂不实现。推理 API 可访问不等于可以训练其权重。

个人模型目录默认保存在 `resources/model_resources.sqlite3`，可通过服务端环境变量 `SCIENCE_MODEL_RESOURCES_DB` 指定位置。数据库包含私有凭据，不应上传或分享；仓库忽略其数据库及辅助文件。这是本地个人资源存储，不是加密密钥托管或多用户权限系统。

资源接口为 `/api/model-resources`；实验引用通过 `/api/experiments/<id>/model-bindings` 维护。资源编辑和绑定保存使用修订校验；原 `llm.yaml` / RL 模型路径保留为未绑定资源时的兼容配置。资源绑定保存与训练参数保存是两个明确操作。

### MAS 模型接入与就绪

正式“调试”入口固定使用 Live，点击“发送调试”才会真实调用模型并可能产生费用。模拟演示位于“实验设置 → 执行环境 → 开发与演示”，不调用模型；数据集采集从“数据与采样”进入。真实 / 模拟输入与本次结果独立保存于当前编辑会话，历史结果单独标记。

调试结果只展示最终答案和按 Agent 划分的输入 / 输出摘要，不提供逐事件日志或页面内详细 JSON 查看器。通过“下载完整 JSON”获取整次运行记录；“下载本页操作日志”仅用于查看浏览器记录的保存与请求状态。后台完整轨迹不受展示简化影响。
...
- 未绑定资源的旧实验使用 `.secrets.env`，其次使用服务启动时的环境 / 仓库 `.env`。绑定个人资源后按资源明确的凭据模式解析，不回退继承其他实验 Key；探测临时输入不自动保存。
- 空密钥允许尝试无鉴权的兼容服务；不自动将真实执行降级为 mock。独立真实运行拒绝没有训练资源注入的 `rl_endpoint`。
- 绑定资源时以服务端引用和资源配置为准；旧实验以 `llm.yaml` 为准。Workflow 保存不会覆盖模型来源，资源绑定缺失或无效时明确报错，不自动换模型。
