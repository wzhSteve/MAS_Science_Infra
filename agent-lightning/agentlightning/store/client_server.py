...
class LightningStoreServer(LightningStore):
    ...
    def _setup_routes(self):
        ...
        @api.get(API_AGL_PREFIX + "/metrics", response_model=Dict[str, Any])
        async def get_training_metrics(  # pyright: ignore[reportUnusedFunction]
            rollout_limit: int = 200,
            step_limit: int = 2000,
        ) -> Dict[str, Any]:
            """Dashboard metrics: episode rewards from spans + VERL step scalars from JSONL."""
            from agentlightning.emitter.reward import find_final_reward
            from agentlightning.utils.training_metrics_sink import (
                discover_metrics_jsonl_candidates,
                read_metrics_jsonl,
            )
            ...
            rollouts = await self.query_rollouts(
                sort_by="end_time",
                sort_order="desc",
                limit=rollout_limit,
                offset=0,
            )
            rollout_rewards: List[Dict[str, Any]] = []
            for rollout in rollouts:
                try:
                    spans = await self.query_spans(rollout.rollout_id, attempt_id="latest")
            ...
            rollout_rewards.reverse()

            candidates = discover_metrics_jsonl_candidates()
            metrics_path = candidates[0] if candidates else os.environ.get("AGL_METRICS_JSONL", "")
            exists = bool(metrics_path) and os.path.isfile(metrics_path)
            training_steps = read_metrics_jsonl(metrics_path, limit=step_limit) if exists else []

            return {
                "rollout_rewards": rollout_rewards,
                "training_steps": training_steps,
                "source": {
                    "metrics_jsonl": metrics_path or None,
                    "exists": exists,
