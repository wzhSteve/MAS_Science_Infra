...
from agentlightning.utils.training_metrics_sink import append_metrics, ensure_metrics_env
...
class AgentLightningTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        metrics_jsonl = ensure_metrics_env(
            project_name=str(self.config.trainer.project_name),
            experiment_name=str(self.config.trainer.experiment_name),
            default_local_dir=str(self.config.trainer.get("default_local_dir", "") or "") or None,
        )
        pprint(f"Training metrics JSONL: {metrics_jsonl}")
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            append_metrics(metrics_jsonl, step=self.global_steps, data=val_metrics)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                append_metrics(metrics_jsonl, step=self.global_steps, data=metrics)
