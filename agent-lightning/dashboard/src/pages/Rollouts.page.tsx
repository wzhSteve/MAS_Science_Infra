...
function toRolloutFromRecord(record: RolloutTableRecord): Rollout {
  const { rolloutId, input, startTime, endTime, mode, resourcesId, status, config, metadata, attempt, finalReward } =
    record;
...
export function RolloutsPage() {
  ...
  const { data: metricsData } = useGetTrainingMetricsQuery(
    { rolloutLimit: 500, stepLimit: 1 },
    { pollingInterval: autoRefreshMs > 0 ? autoRefreshMs : undefined },
  );

  const rewardByRolloutId = useMemo(() => {
    const map = new Map<string, number>();
    for (const point of metricsData?.rolloutRewards ?? []) {
      if (typeof point.finalReward === 'number') {
        map.set(point.rolloutId, point.finalReward);
      }
    }
    return map;
  }, [metricsData?.rolloutRewards]);

  const rolloutsWithRewards = useMemo(() => {
    const items = rolloutsData?.items ?? [];
    return items.map((rollout) => ({
      ...rollout,
      finalReward: rewardByRolloutId.get(rollout.rolloutId) ?? rollout.finalReward ?? null,
    }));
  }, [rolloutsData?.items, rewardByRolloutId]);
