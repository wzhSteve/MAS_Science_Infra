...
const normalizeRollout = (value: unknown): Rollout => {
  const camelized = camelCaseKeys(value) as Rollout & {
    attempt?: unknown;
  ...
  const fromMeta =
    typeof meta.final_reward === 'number'
      ? meta.final_reward
      : typeof meta.finalReward === 'number'
        ? meta.finalReward
        : typeof meta.reward === 'number'
          ? meta.reward
          : null;
  const finalReward =
    typeof camelized.finalReward === 'number'
      ? camelized.finalReward
      : typeof camelized.final_reward === 'number'
        ? camelized.final_reward
        : fromMeta;
