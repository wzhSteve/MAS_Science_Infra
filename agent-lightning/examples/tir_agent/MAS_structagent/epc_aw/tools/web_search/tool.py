...
from MAS.epc_aw.tools.proxy_util import ensure_process_proxy_env, get_requests_proxies
...
# Publish GOOGLE_CHROME_PROXY / WEB_SEARCH_PROXY as HTTP(S)_PROXY for urllib/playwright.
...
class Web_Search_Tool(BaseTool):
    ...
    def _pdf_flow(self, url: str, headers: dict, query: str, max_len: int) -> str:
        ...
        try:
            resp = requests.get(
                url, headers=headers, timeout=20, proxies=get_requests_proxies()
            )
            resp.raise_for_status()
    ...
    def _browser_flow(self, url: str, query: str, max_len: int) -> str:
        ...
        try:
            with sync_playwright() as p:
                ...
                proxies = get_requests_proxies()
                if proxies:
                    launch_kwargs["proxy"] = {"server": proxies["https"]}
