...
from MAS.epc_aw.tools.proxy_util import ensure_process_proxy_env, wikipedia_proxy_env
...
# Prefer tool-scoped proxy; process-wide HTTP_PROXY is skipped when MAS_NO_PROCESS_PROXY=1.
ensure_process_proxy_env()
...
class Wikipedia_Search_Tool(BaseTool):
    ...
    def execute(self, query):
        """
        Searches Wikipedia. If `query` contains multiple comma-separated keywords
        (e.g. "Einstein, relativity, tensor"), each keyword is queried independently
        ...
        #         single_result = self.execute_single(sq)
        #         aggregated_result[sq] = single_result
