...
def _open_construct(
    ...
    agent_items = memory.read("agent", "hub")
    archive.append(
        ExecutionEvent(
...
def _close_construct(
    ...
    agent_item = memory.write(
        MemoryItem(
            scope="agent",
            owner="hub",
            content={"final_answer": raw.final_answer, "n_messages": len(raw.messages)},
        )
    )
    archive.append(
        ExecutionEvent(
            kind=EventKind.MEMORY_WRITE,
            payload={"scope": "agent", "owner": "hub", "id": agent_item.id},
        )
    )
...
def run_mock_episode(
    ...
    *,
    ...
    skill_results = _open_construct(task, archive, spec, memory)
    ...
    if force_error:
        ...
        _close_construct(archive, spec, memory, raw_err, task)
        return raw_err
    ...
    _close_construct(archive, spec, memory, raw, task)
    return raw
...
def run_compiled_episode(
    ...
    while current and hops < max_hops:
        ...
        raw = runner.run(task_i, llm, archive, sub_spec, memory)
        last_raw = raw
        n_search += int(raw.n_search)
        n_python += int(raw.n_python)
