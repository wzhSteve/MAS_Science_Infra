...
class Python_Coder_Tool(BaseTool):
    ...
    @staticmethod
    def _looks_like_python_source(query: str) -> bool:
        """True when the query is already executable code (deterministic inject path)."""
        text = str(query or "").strip()
        if not text:
            return False
        if "```" in text:
            return True
        has_print = bool(re.search(r"\bprint\s*\(", text))
        has_assign = bool(re.search(r"\b\w+\s*=\s*[^=]", text))
        has_import = bool(re.search(r"^\s*(import|from)\s+\w+", text, re.M))
        return has_print and (has_assign or has_import)
    ...
    def execute(self, query):
        ...
        # Deterministic path: solver may inject a ready-made snippet (often
        # semicolon-separated). Run it directly — do not re-ask the LLM.
        if self._looks_like_python_source(query):
            wrapped = query if "```" in str(query) else f"```python\n{query}\n```"
            return self.execute_code_snippet(wrapped)
