...
class SlotGate:
    ...
    def _record_qualifies(
        ...
        if not SlotGate._content_fills_slot(slot, content, record, records, question=question):
            return False
    ...
    def filled_slots(profile: TaskProfile, evidence_records: List[Dict[str, Any]]) -> Set[str]:
        ...
        for slot in profile.slots:
            ...
            for rec in evidence_records:
                if SlotGate._record_qualifies(rec, slot, evidence_records, question=question):
                    filled.add(slot.name)
                    break
        return filled
