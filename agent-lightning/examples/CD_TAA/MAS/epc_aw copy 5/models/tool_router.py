...
class ToolRouter:
    ...
    def validate(
        ...
        # Hard-block compute tools on acquisition — prevents hallucination path
        # where Python fabricates facts (Kipchoge pace / Nature counts).
        if kind == "acquisition" and tool_name in COMPUTE_TOOLS:
            alt = ToolRouter._pick_alternative(available_tools, kind)
            return alt or tool_name, False, kind
        ...
        # Hard tool-class enforcement for compute/visual/acquisition.
        if tool_name not in allowed and kind in ("compute", "visual", "acquisition"):
            alt = ToolRouter._pick_alternative(available_tools, kind)
            return alt or tool_name, False, kind

        appropriate = tool_name in allowed
