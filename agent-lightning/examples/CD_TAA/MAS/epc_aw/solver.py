...
class Solver(CommandOpsMixin, AnswerGateMixin, InterventionMixin, ToolPolicyMixin, PlanControllerMixin, EvidenceBinderMixin):
    def __init__(
        ...
        apply_flags = getattr(self.system_memory, "apply_ablation_memory_flags", None)
        if callable(apply_flags):
            apply_flags(self.ablation)
    ...
    def _analyze_query(self, question: str, image_path: Optional[str], json_data: Dict[str, Any], start_time: float) -> None:
        ...
        self._seed_subgoal_contracts_from_outline(execution_outline, profile)
    ...
    def _seed_subgoal_contracts_from_outline(
        self,
        outline: Dict[str, Any],
        profile: Any,
    ) -> None:
        """Project SlotGate required_fact_contract onto causal-graph Subgoal nodes."""
        if not outline or not profile:
            return
        graph = getattr(self.system_memory, "causal_graph", None)
        if graph is None:
            return
        contract = SlotGate.required_fact_contract(profile)
        required_all = list(contract.keys())
        attach = getattr(graph, "attach_subgoal_contract", None)
        if not callable(attach):
            return
        for _key, step_text in outline.items():
            text = str(step_text)
            kind = infer_subgoal_kind(text)
            preferred: List[str] = []
            for spec in contract.values():
                preferred.extend(spec.get("preferred_tools") or [])
            # Keep order-unique tools.
            seen = set()
            preferred_tools = []
            ...
            attach(
                text,
                required_fact_names=required_all if kind != "verify" else [],
                preferred_tools=preferred_tools,
                acquisition=kind != "verify",
            )

    def _sync_outline_projection_from_graph(self) -> None:
        """When struct stop gate is on, refresh outline from open acquisition subgoals."""
        ablation = getattr(self, "ablation", None)
        enabled = True
        if ablation is not None:
            enabled = bool(getattr(ablation, "enable_struct_stop_gate", True))
        else:
            enabled = bool(getattr(self.system_memory, "enable_struct_stop_gate", True))
        if not enabled:
            return
        graph = self.system_memory.causal_graph
        project = getattr(graph, "project_outline_from_open_subgoals", None)
        if not callable(project):
            return
        projected = project(self.system_memory.current_state_valid_facts())
        if projected:
            self.system_memory.set_outline(projected)
    ...
    def _run_verification(
        ...
        # Commit stage (partial): slot harvest/persist via commit_progress only.
        # Full obtained commit stays on complete/incomplete handlers so L2
        # candidate-set verifications do not mint duplicate live facts.
    ...
    def _handle_subgoal_complete(
        ...
        # Sole live-fact commit for a completed subgoal (commit_progress).
        self._record_obtained_information(exec_step, verification, step_ctx)
        # Outline is a projection: mark done after verified commit.
        ...
        if self.can_attempt_done(question, verification.analysis):
            if self._answer_sanity_pass(question):
                candidate = None
                profile = self.system_memory.get_task_profile()
                records = self.system_memory.slot_evidence_records(
                    require_live_verified=self._struct_stop_enabled()
                )
                if profile:
                    candidate = SlotGate.extract_final_answer(profile, records)
                if not candidate:
                    candidate = str(
                        self.system_memory.get_obtained_information_for_prompt() or ""
                    )[:500]
                if self._audit_and_maybe_invalidate_final(
                    question, candidate or "", step=exec_step,
                ):
                    print(
                        f"\n==> 🎯 Execution #{exec_step}: "
                        f"can_attempt_done + FinalAudit PASS — STOP\n"
                    )
                    return "stop"
                print(
                    f"\n==> 🔎 Execution #{exec_step}: FinalAudit FAIL — CONTINUE\n"
                )
                return "continue"
            return "continue"
        # Keep outline aligned with open acquisition subgoals (graph projection).
        self._sync_outline_projection_from_graph()
    ...
    def solve(self, question: str, image_path: Optional[str] = None):
        ...
        propose→act→verify→commit loop (single progress commit API):
          init → analyze (+ seed Subgoal contracts) → while budget:
            if can_attempt_done: finalize + FinalAudit; break or invalidate+continue
            propose/act ← outline head OR graph-open acquisition projection
            verify → commit_progress (EvidenceBinder / obtained)
            if satisfied: mark outline projection done; SlotGate / FinalAudit
            else: FailureAttribution → L1→L2a→L2b→L3; success re-enters verify→commit
          persist memory (Cap/Inv evolve, verified-only)
        """
        ...
        # ===== 主循环：propose → act → verify → commit =====
        # 进度权威：commit_progress + CausalMemoryGraph；outline 仅为投影。
        # 验证判 SUBGOAL_INCOMPLETE 时进入 FailureAttribution 介入梯。
        while exec_step < self.max_steps and (time.time() - query_start_time) < self.max_time:
            ...
            # Loop-head DONE check: verified slots + open-acquisition clear + audit.
            if self.can_attempt_done(question, context_verification) and self._answer_sanity_pass(
                question
            ):
                candidate = None
                profile = self.system_memory.get_task_profile()
                ...
                if self._audit_and_maybe_invalidate_final(
                    ...
                    print(
                        f"\n==> 🎯 Loop-head can_attempt_done + FinalAudit PASS — STOP "
                        f"(exec_step={exec_step})\n"
                    )
                    last_task_conclusion = "STOP"
                    break
                print(
                    f"\n==> 🔎 Loop-head FinalAudit FAIL — CONTINUE "
                    f"(exec_step={exec_step})\n"
                )
            ...
            if step_key is None:
                # Outline empty: prefer graph-open acquisition projection over recovery.
                self._sync_outline_projection_from_graph()
                step_key, target_information = self._get_first_outline_step()
                if step_key is not None:
                    print(
                        "\n==> 📐 Outline projected from open acquisition subgoals\n"
                    )
                    # Fall through to execute the projected head step.
                elif self.can_attempt_done(question) and self._answer_sanity_pass(question):
                    candidate = None
                    profile = self.system_memory.get_task_profile()
                    records = self.system_memory.slot_evidence_records(
                        require_live_verified=self._struct_stop_enabled()
            ...
            # Propose + Act：Planner 提议工具/子目标，Executor 生成命令并执行。
            ...
            # Verify → Commit：Diagnoser 验证；EvidenceBinder 经 commit_progress 写入。
            ...
            # COMPLETE 必须落到 _handle_subgoal_complete（sanity + FinalAudit），不可在此 break。
            ...
            if verification.step_conclusion == "SUBGOAL_INCOMPLETE":
                ...
                if action == "complete":
                    ...
                    if task_action == "stop" and self.can_attempt_done(
                        question, verification.analysis,
                    ):
                        context_verification = verification.analysis
                        break
                ...
                # Cause-Changed gate terminated the intervention loop.
                if action == "terminate":
                    print(
        ...
        if 'direct' in self.output_types:
            evidence_verified = self.can_attempt_done(question, context_verification)
            profile = self.system_memory.get_task_profile()
            records = self.system_memory.slot_evidence_records(
                require_live_verified=self._struct_stop_enabled()
            )
...
def construct_solver(
    ...
    """Build Solver. ``base_url`` routes all LLM clients to a local OpenAI-compatible
    server (VERL/vLLM) instead of CD_TAA ``.env`` remote endpoints.
    """
