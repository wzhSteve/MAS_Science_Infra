...
def _tool_proxies():
    """Optional HTTP(S) proxy for page fetches (SSH RemoteForward / Clash).

    Reads GOOGLE_CHROME_PROXY / WEB_SEARCH_PROXY / CHROME_PROXY — same as google_search.
    Does NOT read process-wide http_proxy (avoids breaking VERL local endpoints).
    """
    proxy = (
        os.getenv("GOOGLE_CHROME_PROXY", "").strip()
        or os.getenv("WEB_SEARCH_PROXY", "").strip()
...
def _playwright_proxy():
    proxies = _tool_proxies()
    if not proxies:
        return None
    return {"server": proxies["https"]}
...
You are an expert AI assistant. Your task is to answer the user's query based **exclusively** on the provided reference chunks.

## Hard Rules
1. Use **only** facts that appear in the reference chunks. No external knowledge.
2. Prefer **verbatim quotes** for numbers, names, dates, and character symbols. Do **not** paraphrase figures.
3. Do **not** invent letters, punctuation names, or counts that are absent from the chunks.
4. If the chunks do not contain enough information, respond exactly:
   The search results do not provide information that answers this query.

## Output Format
Answer:
<one short paragraph that quotes the key supporting phrases from the chunks>
...
NUMERIC_SELECT_PROMPT = """
You are selecting ONE numeric fact from candidates. Do not invent numbers.
...
Rules:
- Return ONLY one candidate index (0-based integer), OR the word insufficient
- Choose the candidate whose quote best answers the query (units/scope must match)
- Never invent a number not listed
...
class Web_Search_Tool(BaseTool):
    ...
    def _get_website_content(self, url: str, query: str, max_len: int = 4000, raw: bool = False) -> str:
        """
        Main entry:
          - url: web page or PDF link
          - query: user's query (used to find relevant content)
          - max_len: hard cap on returned characters (default 4000)
          - raw: when True, return the joined relevant paragraphs WITHOUT
            extractive summarization, so the caller (execute()) can do the
            authoritative embedding-based ranking. When False (legacy),
            return an extractive TF-cosine summary (used by Google_Search_Tool
            and other direct callers).
        Returns a relevance-selected string (<= max_len chars).
        """
        max_len = int(max_len)
        max_len = min(max_len, 4000)  # hard cap, raised from 1000 for detail extraction
        ...
        try:
            head = requests.head(
                url, headers=headers, timeout=8, allow_redirects=True, proxies=proxies
            )
        ...
        try:
            ...
            if raw:
                # Return joined relevant paragraphs (no extractive summary);
                # the embedding ranker in execute() is the authoritative selector.
                return self._safe_truncate(" ".join(top_pars), max_len)
    ...
    def _construct_final_output(self, query, reference_information):
        """
        Construct the final output from the top chunks (extractive paraphrase ban).
        """
    ...
    def _select_numeric_via_llm(self, query: str, candidates: list):
        """Ask LLM to pick a candidate index; never invent values."""
        if not candidates:
            return None
        if len(candidates) == 1:
            return candidates[0]
        try:
            prompt = NUMERIC_SELECT_PROMPT.format(
                query=query,
                candidates=json.dumps(candidates, ensure_ascii=False, indent=2),
            )
            raw = str(self.llm_engine(prompt) or "").strip().lower()
            ...
            pass
        return select_best_numeric_fact(candidates, query)

    def _build_extractive_output(self, query: str, top_chunks: list, url: str) -> str:
        """Numeric/character: no synthesize; entity: quote-grounded RAG + optional facts."""
        ...
        if intent == "numeric":
            ...
            for chunk in top_chunks:
                candidates.extend(extract_numeric_candidates(chunk, url=url))
            # Dedup by value+unit
            deduped = []
            seen = set()
            for c in candidates:
                key = (str(c.get("value")), c.get("unit"), str(c.get("quote"))[:60])
                if key in seen:
                    continue
                seen.add(key)
                deduped.append(c)
            chosen = self._select_numeric_via_llm(query, deduped[:12])
            if not chosen:
                chosen = select_best_numeric_fact(deduped, query)
            if not chosen:
            ...
            human = str(chosen.get("quote") or chosen.get("value"))
            return format_tool_output(human, [chosen], refusal=False)

        if intent == "character_name":
            facts = extract_character_candidates(joined, url=url)
            if not facts:
                return format_tool_output(
                    "The search results do not provide information that answers this query.",
                    [],
            ...
            # Prefer backtick/backquote when Unlambda-style query mentions applicative
            qlow = str(query or "").lower()
            ordered = list(facts)
            if any(k in qlow for k in ("unlambda", "applicative", "backtick", "backquote")):
                ordered.sort(
                    key=lambda f: 0 if f.get("value") == "backtick" else 1,
                )
            human = str(ordered[0].get("quote") or ordered[0].get("value"))
            return format_tool_output(human, [ordered[0]], refusal=False)

        # entity / general: keep RAG but attach extractive facts when present
        reference_string = self._concatenate_chunks(list(top_chunks))
        summary = self._construct_final_output(query, reference_string)
        facts = extract_numeric_candidates(joined, url=url)[:5]
        facts.extend(extract_character_candidates(joined, url=url)[:3])
        ...
        if isinstance(summary, str) and summary.strip().lower().startswith(refuse):
            return format_tool_output(summary, [], refusal=True)
        return format_tool_output(str(summary or ""), facts, refusal=not facts and not summary)

    def execute(self, query, url):
        try:
            # step 1: get content from the website (raw paragraphs so the
            # embedding ranker below is the authoritative selector, avoiding
            # the legacy double-summarize where TF-cosine pre-summarized the
            # text that embedding then re-ranked).
            website_content = self._get_website_content(url=url, query=query, raw=True)
            ...
            # step 5: extractive / quote-grounded output (+ STRUCTURED block)
            return self._build_extractive_output(query, top_chunks, url)
        except Exception as e:
