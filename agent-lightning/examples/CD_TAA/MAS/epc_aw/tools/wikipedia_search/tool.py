...
# NOTE: Do NOT import Web_Search_Tool at module top-level.
# Initializer uses inspect.getmembers on this module; a top-level import would
# incorrectly register Web_Search_Tool when only Wikipedia_Search_Tool is enabled.
...
def select_relevant_queries(original_query: str, query_candidates: list[str], llm_engine):
    candidate_list = list(query_candidates)
    query_candidates_text = "\n".join(
        [f"{i}. {query}" for i, query in enumerate(candidate_list)]
    )
    ...
    prompt = f"""
                ...
                Query Candidates from Wikipedia Search: `{query_candidates_text}`
    ...
    try:
        ...
        if isinstance(response, Select_Relevant_Queries):
            ...
            matched_query_ids = list(response.matched_query_ids)
        elif isinstance(response, dict):
            matched_queries = list(response.get("matched_queries") or [])
            matched_query_ids = list(response.get("matched_query_ids") or [])
        else:
            text = str(response)
            mq_match = re.search(r"Matched Queries:\s*(.*)", text)
            matched_queries = (
                [q.strip() for q in mq_match.group(1).split(",")]
                if mq_match else []
            )
            id_match = re.search(r"Matched Query IDs:\s*\[(.*?)\]", text)
            matched_query_ids = (
                [int(i.strip()) for i in id_match.group(1).split(",") if i.strip()]
                if id_match else []
            )

        matched_query_ids = [
            int(i) for i in matched_query_ids
            if str(i).lstrip("-").isdigit() and 0 <= int(i) < len(candidate_list)
        ][:3]
        if not matched_query_ids and candidate_list:
            query_terms = set(re.findall(r"[a-z0-9]+", original_query.lower()))
            scored = []
            for idx, title in enumerate(candidate_list):
                title_terms = set(re.findall(r"[a-z0-9]+", title.lower()))
                scored.append((len(query_terms & title_terms), idx))
            scored.sort(key=lambda item: (-item[0], item[1]))
            matched_query_ids = [idx for score, idx in scored[:3] if score > 0]
            if not matched_query_ids:
                matched_query_ids = [0]
            matched_queries = [candidate_list[i] for i in matched_query_ids]
...
class Wikipedia_Search_Tool(BaseTool):
    def __init__(self, model_string=os.getenv("MODEL_Name")):
        super().__init__(
            ...
            tool_description="A high-precision lookup tool specialized for encyclopedic entities such as notable people, historical events, organizations, locations, physical objects, and well-defined academic or technical concepts. The tool accepts exactly one entity-level keyword per query and retrieves information from the most relevant Wikipedia page. It is not used for vague topics, multi-keyword queries, or non-encyclopedic requests.",
            tool_version="1.0.0",
            input_types={
    ...
    def search_wikipedia(self, query, max_length=4000, max_pages=10):
        """
    ...
    def execute(self, query):
        """
        Searches Wikipedia. If `query` contains multiple comma-separated
        keywords (e.g. "Einstein, relativity, tensor"), each keyword is
        queried independently and results are concatenated under separate
        keys, so downstream consumers can tell which sub-query answered what.

        Parameters:
            query (str): Search query or comma-separated set of queries.

        Returns:
            dict: Aggregated search results for one or multiple queries.
        """
        # Multi-keyword split: comma-separated entities are independent
        # lookups; concatenating them into one wiki title (the old
        # `replace(", ", "_")` hack) produced invalid titles and empty hits.
        if "," in query:
            sub_queries = [q.strip() for q in query.split(",") if q.strip()]
            if len(sub_queries) > 1:
                aggregated = {}
                for sq in sub_queries:
                    aggregated[sq] = self.execute_single(sq)
                return {
    ...
    def execute_single(self, query):
        """
        Performs the original single-keyword Wikipedia search.
        (Directly extracted from your previous execute logic.)
        """
        ...
        # Step 2: LLM-based title matching
        matched_queries, matched_query_ids = select_relevant_queries(
            query, titles, self.llm_engine
        )

        valid_ids = [
            i for i in matched_query_ids
            if isinstance(i, int) and 0 <= i < len(search_results)
        ]
        if not valid_ids and search_results:
            valid_ids = [0]
        ...
        # Step 3: Web RAG fetching (lazy import — see module docstring note)
        try:
...
if __name__ == "__main__":
    ...
    tool = Wikipedia_Search_Tool(model_string=os.getenv("MODEL_Name"))
