...
class Initializer:
    ...
    def _allowed_external_tool_names(self) -> set:
        """Resolve enabled_tools (short or long names) to external TOOL_NAME set."""
        short_to_long = {}
        if hasattr(self, "tool_name_mapping"):
            short_to_long = self.tool_name_mapping.get("short_to_long", {}) or {}
        allowed = set()
        for tool in self.enabled_tools or []:
            if not tool or tool == "all":
                continue
            allowed.add(short_to_long.get(tool, tool))
            allowed.add(tool)
        return allowed
    ...
    def load_tools_and_get_metadata(self) -> Dict[str, Any]:
        ...
        allowed_tools = self._allowed_external_tool_names()
        strict_filter = bool(allowed_tools) and "all" not in (self.enabled_tools or [])
        short_to_long = self.tool_name_mapping.get("short_to_long", {}) or {}
        long_to_internal = self.tool_name_mapping.get("long_to_internal", {}) or {}
                    ...
                    for name, obj in inspect.getmembers(module):
                        if inspect.isclass(obj) and name.endswith('Tool') and name != 'BaseTool':
                            ...
                            try:
                                ...
                                # Prefer the class that actually belongs to this directory.
                                # Imported sibling Tool classes (e.g. Web_Search in wikipedia module)
                                # must not be registered here.
                                candidate_external = short_to_long.get(name, name)
                                if candidate_external in long_to_internal:
                                    if long_to_internal[candidate_external]["dir_name"] != current_dir_name:
                                        continue
                                if strict_filter and candidate_external not in allowed_tools and name not in allowed_tools:
                                    continue
                                ...
                                if strict_filter and metadata_key not in allowed_tools and name not in allowed_tools:
                                    continue
    ...
    def _set_up_tools(self) -> None:
        ...
        # Strict filter: never expose tools outside enabled_tools (even if a sibling
        # Tool class leaked into toolbox_metadata via import side-effects).
        allowed = self._allowed_external_tool_names()
        if allowed and "all" not in (self.enabled_tools or []):
            self.available_tools = [t for t in self.available_tools if t in allowed]
            self.toolbox_metadata = {
                t: self.toolbox_metadata[t]
                for t in self.available_tools
                if t in self.toolbox_metadata
            }
