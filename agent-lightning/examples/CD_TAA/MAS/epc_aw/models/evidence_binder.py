...
class EvidenceBinderMixin:
    ...
    All live fact / slot writes go through ``SystemMemory.commit_progress``
    (alias of commit_verified_memory + optional outline mark-done).
    """

    def _progress_commit(self):
        mem = self.system_memory
        commit = getattr(mem, "commit_progress", None)
        if callable(commit):
            return commit
        return getattr(mem, "commit_verified_memory", None)
    ...
    def after_verify(
        ...
        *,
        ...
        elif getattr(self.system_memory, "enable_verified_memory_commit", False):
            # Candidate / shadow verifies still emit rejected events without writing facts.
            # Skip extra rejection if a satisfied commit already landed via harvest.
            if not had or not verification.subgoal_complete:
                self._commit_rejection_without_evidence(
                    step_count, verification, step_ctx, reason="record_obtained=False"
                )
        verification.had_slot_delta = had
        return verification

    def _commit_rejection_without_evidence(
        self,
        step_count: int,
        verification: VerificationResult,
        step_ctx: StepContext,
        *,
        ...
        commit(
            MemoryCommitEvent(
                ...
                step=step_count,
                reason=reason or (verification.analysis or "rejected"),
                outline_step=str(step_ctx.step_key or ""),
                tool_name=step_ctx.tool_name,
                attributed_to="verifier",
                recovery_hint="Retry with different parameters or switch tool.",
            )
        )

    def _commit_slot_bindings(
        self,
        *,
        content: str,
        ...
        """Commit harvested/persisted slot bindings via the sole progress API."""
        if not bindings or not str(content or "").strip():
            return False
        kind = map_verification_to_commit_kind(
            step_conclusion=verification.step_conclusion,
            subgoal_complete=bool(verification.subgoal_complete),
            evidence_type=verification.evidence_type or "",
            claim_type=claim_type,
            compute_real=compute_real,
        )
        # Slot harvest on incomplete verification must not mint live facts.
        if kind not in {CommitKind.SATISFIED.value, CommitKind.VALUE_COMMITTED.value}:
            if not verification.subgoal_complete:
                return False
            kind = (
                CommitKind.VALUE_COMMITTED.value
                if compute_real
                else CommitKind.SATISFIED.value
            )

        commit = self._progress_commit()
        use_commit = getattr(self.system_memory, "enable_verified_memory_commit", False)
        if use_commit and callable(commit):
            result = commit(
                MemoryCommitEvent(
                    kind=kind,
                    step=step_count,
                    reason=(verification.analysis or "slot_bindings")[:500],
                    outline_step=str(step_ctx.step_key or ""),
                    facts=dict(bindings),
                    tool_name=step_ctx.tool_name,
                    attributed_to="verifier",
        ...
        # Legacy path when verified-memory commit is disabled.
    ...
    def _harvest_slots_from_executor(
        ...
        if step_ctx.tool_name == "Base_Generator_Tool":
            quality = "inferred"
        return self._commit_slot_bindings(
            content=content,
            bindings=bindings,
            step_ctx=step_ctx,
    ...
    def _persist_slot_deltas(
        ...
        if verification.evidence_type not in ("ABSENCE", "EMPTY", "ERROR"):
            term_m = re.search(
            ...
            is_multiple_choice = bool(
                profile and any(
                    slot.name == "final_answer"
                    and slot.slot_type == "multiple_choice"
            ...
            if is_multiple_choice:
                canonical = _canonicalize_multiple_choice_answer(
                    question,
                    analysis,
                    [obtained, executor_blob],
                )
                if canonical:
                    inferred["final_answer"] = canonical
    ...
    def _record_obtained_information(
        ...
        kind = map_verification_to_commit_kind(
            step_conclusion=verification.step_conclusion,
            subgoal_complete=bool(verification.subgoal_complete and should_record),
            evidence_type=verification.evidence_type or "",
            claim_type=claim_type,
            compute_real=compute_real,
        )
        # Incomplete / not-recordable paths are rejected commits (no facts).
        if not should_record:
            kind = CommitKind.REJECTED.value

        use_commit = getattr(self.system_memory, "enable_verified_memory_commit", False)
        commit_fn = self._progress_commit()
        if use_commit and callable(commit_fn):
            facts = dict(slot_bindings)
            if (
                kind in {CommitKind.SATISFIED.value, CommitKind.VALUE_COMMITTED.value}
                and info
                and not facts
            ):
                facts = {"obtained": info}
            ...
            result = commit_fn(
                MemoryCommitEvent(
                    kind=kind,
                    step=step_count,
                    reason=(verification.analysis or kind)[:500],
                    outline_step=str(step_ctx.step_key or ""),
                    facts=facts,
                    tool_name=step_ctx.tool_name,
                    attributed_to="verifier",
                    recovery_hint=(
                        "Retry with different parameters or switch tool."
                        if kind == CommitKind.REJECTED.value
                        else ""
                    ),
                ),
            ...
            if verification.slot_updates and result.get("committed"):
                profile = self.system_memory.get_task_profile()
                if profile:
            ...
            if result.get("committed"):
                print(
                    f"\n==> ℹ️ Step {step_count}: Verified memory commit "
                    f"({result.get('event_kind')}).\n"
                )
                if info:
                    print(f"\n[New Obtained Information]:\n{info}\n")
            elif should_record:
                print(
                    f"\n==> ℹ️ Step {step_count}: Duplicate/low-quality or non-fact "
                    f"record skipped (kind={result.get('event_kind')}).\n"
                )
            else:
