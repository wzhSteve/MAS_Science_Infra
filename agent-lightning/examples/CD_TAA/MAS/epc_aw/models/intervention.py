...
# StructAgent FailureAttribution → first-hop recommendation / rung.
ATTRIBUTION_TO_RECOMMENDATION: Dict[str, str] = {
    "actor": "retry_with_different_parameters",
    "tool_environment": "switch_tool",
    "planner": "decompose_goal",
    "verifier": "revise_belief",  # re-verify / tighten contract; not hard L3 escalate
}
ACTOR_BURST_SIZE = 3
...
class StepInterventionState:
    ...
    # Graph-backed knowledge-gain snapshots (StructAgent alignment).
    last_live_evidence_count: Dict[str, int] = field(default_factory=dict)
    last_valid_fact_keys: Dict[str, Set[str]] = field(default_factory=dict)
    last_diagnosis_edge_count: Dict[str, int] = field(default_factory=dict)
    actor_burst_used: Dict[str, int] = field(default_factory=dict)
    last_attribution: Dict[str, str] = field(default_factory=dict)
...
class InterventionMixin:
    ...
    def _record_causal_effect(
        ...
        # Attach SlotGate-derived VerificationSpec-like contract onto Subgoal node.
        self._attach_subgoal_contract_for_step(step_ctx)
    ...
    def _attach_subgoal_contract_for_step(self, step_ctx: "StepContext") -> None:
        ...
        missing = []
        try:
            records = self.system_memory.slot_evidence_records(
                require_live_verified=bool(
                    getattr(self.system_memory, "enable_struct_stop_gate", True)
                )
            )
            missing = profile.missing_slot_names(
                records,
                valid_facts=self.system_memory.current_state_valid_facts(),
                require_live_verified=True,
            )
        except Exception:
            missing = [s.name for s in profile.slots if s.required]
        required = missing or list(contract.keys())
        preferred: List[str] = []
        for name in required:
            preferred.extend(contract.get(name, {}).get("preferred_tools") or [])
        ...
        if step_ctx.tool_name and step_ctx.tool_name not in preferred_tools:
            preferred_tools.insert(0, step_ctx.tool_name)
        graph = self.system_memory.causal_graph
        attach = getattr(graph, "attach_subgoal_contract", None)
        if callable(attach):
            kind = infer_subgoal_kind(step_ctx.sub_goal or step_ctx.target_information or "")
            attach(
                step_ctx.sub_goal or step_ctx.target_information or "",
                required_fact_names=required,
                preferred_tools=preferred_tools,
                acquisition=kind != "verify",
            )
    ...
    def _graph_knowledge_snapshot(self) -> Tuple[int, Set[str], int]:
        """Return (live_evidence_count, valid_fact_keys, diagnosis_edge_count)."""
        mem = self.system_memory
        graph = getattr(mem, "causal_graph", None)
        live_count = 0
        fact_keys: Set[str] = set()
        diag_count = 0
        if graph is not None:
            count_live = getattr(graph, "count_live_evidence_nodes", None)
            if callable(count_live):
                live_count = int(count_live())
            count_diag = getattr(graph, "count_diagnosis_edges", None)
            if callable(count_diag):
                diag_count = int(count_diag())
            facts = getattr(graph, "current_valid_facts", lambda: {})()
            fact_keys = set(str(k) for k in (facts or {}).keys())
        # Prefer verified ledger fact keys when present.
        ledger_facts = getattr(mem, "current_state_valid_facts", lambda: {})()
        if ledger_facts:
            fact_keys |= set(str(k) for k in ledger_facts.keys())
        return live_count, fact_keys, diag_count
    ...
    def _compute_intervention_gains(
        ...
        Exploration Gain (ΔAction): command/tool fingerprint change.

        Knowledge Gain prefers causal-graph / verified-ledger deltas
        (ΔEvidence ∪ ΔValidFacts ∪ ΔDiagnosisEdges), falling back to the
        legacy evidence_type/slot/hypothesis signal deltas.
        """
        live_count, fact_keys, diag_count = self._graph_knowledge_snapshot()
        first_graph = step_key not in tracker.last_live_evidence_count
        first_signal = not step_key or step_key not in tracker.last_hypothesis
        if first_graph and first_signal:
            tracker.last_live_evidence_count[step_key] = live_count
            tracker.last_valid_fact_keys[step_key] = set(fact_keys)
            tracker.last_diagnosis_edge_count[step_key] = diag_count
            return 1, 1
        ...
        knowledge = 0
        if live_count != tracker.last_live_evidence_count.get(step_key, live_count):
            knowledge += 1
        if fact_keys != tracker.last_valid_fact_keys.get(step_key, fact_keys):
            knowledge += 1
        if diag_count != tracker.last_diagnosis_edge_count.get(step_key, diag_count):
            knowledge += 1

        # Legacy signal deltas remain as soft contributors.
        cur_et = (
    ...
    def _struct_intervention_routing_enabled(self) -> bool:
        ablation = getattr(self, "ablation", None)
        if ablation is not None:
            return bool(getattr(ablation, "enable_struct_intervention_routing", True))
        return bool(
            getattr(self.system_memory, "enable_struct_intervention_routing", True)
        )

    def _apply_failure_attribution_routing(
        self,
        question: str,
        step_ctx: StepContext,
        verification: VerificationResult,
        ...
        """Attribute failure and rewrite recommendation for first-hop rung."""
        if not self._struct_intervention_routing_enabled():
            return diagnostic_signal

        diagnoser = getattr(self, "diagnoser", None)
        if diagnoser is None:
            return diagnostic_signal
        attr_fn = getattr(diagnoser, "attribute_failure", None)
        ...
        attributed_to = str(
            getattr(attribution, "attributed_to", None)
            or (attribution.get("attributed_to") if isinstance(attribution, dict) else "actor")
        )
        reason = str(
            getattr(attribution, "reason", None)
            or (attribution.get("reason") if isinstance(attribution, dict) else "")
        )
        recovery = str(
            getattr(attribution, "recovery_hint", None)
            or (attribution.get("recovery_hint") if isinstance(attribution, dict) else "")
        )

        signal = dict(diagnostic_signal)
        signal["failure_attribution"] = {
            "attributed_to": attributed_to,
            "reason": reason,
            "recovery_hint": recovery,
        }
        step_key = step_ctx.step_key or ""
        if step_key:
            tracker.last_attribution[step_key] = attributed_to

        # Persist into verified ledger recent_failures (rejected event).
        from MAS.epc_aw.models.verified_memory import CommitKind, MemoryCommitEvent

        commit = getattr(self.system_memory, "commit_progress", None) or getattr(
            self.system_memory, "commit_verified_memory", None
        )
        if callable(commit) and getattr(
            self.system_memory, "enable_verified_memory_commit", True
        ):
            commit(
                MemoryCommitEvent(
                    kind=CommitKind.REJECTED.value,
                    step=exec_step,
                    reason=reason or "subgoal incomplete",
                    outline_step=str(step_ctx.step_key or ""),
        ...
        # Annotate latest intervention node on the causal graph when present.
        graph = getattr(self.system_memory, "causal_graph", None)
        if graph is not None:
            for node_id, node in reversed(list(graph.nodes.items())):
                if node.get("type") == "Intervention":
                    node["attributed_to"] = attributed_to
                    node["recovery_hint"] = recovery
                    break

        recommendation = ATTRIBUTION_TO_RECOMMENDATION.get(
            attributed_to, "retry_with_different_parameters"
        )
        # Actor burst: keep L2a while burst budget remains.
        if attributed_to == "actor" and step_key:
            used = tracker.actor_burst_used.get(step_key, 0)
            if used < ACTOR_BURST_SIZE:
                tracker.actor_burst_used[step_key] = used + 1
                recommendation = "retry_with_different_parameters"
            else:
                recommendation = "switch_tool"
        # Verifier attribution: force revise_belief soft path, do not jump to modify_state.
        if attributed_to == "verifier":
            recommendation = "revise_belief"

        signal["recommendation"] = recommendation
        if self.verbose:
            print(
                f"[FailureAttribution] to={attributed_to} → {recommendation} | "
                f"{reason[:160]}"
            )
        return signal
    ...
    def _handle_subgoal_incomplete(
        ...
        if diagnostic_signal:
            ...
            # StructAgent FailureAttribution → first-hop rung (before L2 dispatch).
            diagnostic_signal = self._apply_failure_attribution_routing(
                question, step_ctx, verification, diagnostic_signal, tracker, exec_step,
            )
        ...
        _attribution = (
            (diagnostic_signal.get("failure_attribution") or {}).get("attributed_to")
            or tracker.last_attribution.get(step_ctx.step_key or "", "")
        )
        _attr_skip_l2a = (
            self._struct_intervention_routing_enabled()
            and _attribution in {"planner", "tool_environment", "verifier"}
        )
        skip_param_retry = (
            not enable_l2a
            or _attr_skip_l2a
        ...
        # Planner attribution: skip L2b as well and go straight to L3.
        if (
            self._struct_intervention_routing_enabled()
            and _attribution == "planner"
        ):
            enable_l2b = False
        # Verifier attribution: skip tool switch; revise_belief / re-verify via L3.
        if (
            self._struct_intervention_routing_enabled()
            and _attribution == "verifier"
        ):
            enable_l2b = False
