...
class Diagnoser:
    ...
    def _request_context_verification(
        self,
        input_data: List[Any],
    ) -> Optional[Dict[str, Any]]:
        """Request and parse verification, retrying once without response_format."""
        attempts = (
            ("structured", {"response_format": ContextVerification}),
            ("plain-json", {}),
        )
        for attempt_name, kwargs in attempts:
            response = self.llm_engine_fixed(input_data, **kwargs)
            try:
                if response is None:
                ...
                if isinstance(response, dict) and response.get("error"):
                    raise ValueError(
                        f"engine error: {response.get('error')}: "
                        f"{response.get('message', '')}"
                    )
                parsed = self._parse_verification_response(response)
                required = {"Analysis", "Subgoal_Conclusion"}
                missing = required - set(parsed)
                if missing:
                    raise ValueError(
                        f"missing required verification fields: {sorted(missing)}"
                    )
                return parsed
                ...
                if len(preview) > 300:
                    preview = preview[:297] + "..."
                print(
                    f"\n==> ⚠️ ContextVerification {attempt_name} response invalid "
                    f"({type(exc).__name__}: {exc}); "
                    f"type={type(response).__name__}, response={preview}\n"
                )
        return None

    def _conservative_verification_fallback(
        self,
        result_executor: Any,
    ) -> Dict[str, Any]:
        """Continue safely when both verifier responses are unavailable."""
        usable = self._has_usable_result(result_executor)
        result_text = str(result_executor or "").strip()
        obtained = result_text[:500] if usable else ""
        evidence_type = self._detect_evidence_type(result_executor, "")
        return {
            "Analysis": (
                "Verification response unavailable after one retry; "
                "conservatively continuing without filling answer slots."
            ),
            "Subgoal_Conclusion": "SUBGOAL_INCOMPLETE",
            "New_Obtained_Information_Flag": bool(usable and obtained),
            "New_Obtained_Information": obtained,
            "Conclusion": "CONTINUE",
            "Slot_Updates": [],
            "Evidence_Type": evidence_type,
    ...
    def _reconcile_task_conclusion(
        ...
        if self.system_memory:
            ...
            getter = getattr(self.system_memory, "slot_evidence_records", None)
            if callable(getter):
                records = getter(require_live_verified=require_live)
            else:
                records = self.system_memory.evidence_records
            valid_facts = getattr(self.system_memory, "current_state_valid_facts", lambda: {})()

        if profile:
            answer_verified = SlotGate.can_stop(
                profile,
                records,
                valid_facts=valid_facts,
                require_live_verified=require_live,
            )
        else:
    ...
    def verificate_context(self, question: str, image: str, target_information: str, outline: Dict[str, Any], result_executor: str, step_count: int = 0, obtained_information: Any = None, sub_goal: str = "", tool_name: str = "", intervention_context: Optional[Dict[str, Any]] = None, current_outline_step: Optional[str] = None) -> Any:
        ...
        parsed = self._request_context_verification(input_data)
        if parsed is None:
            parsed = self._conservative_verification_fallback(result_executor)
    ...
    def attribute_failure(
        ...
        *,
        ...
        """Map incomplete verification to StructAgent-style failure attribution.

        Prefer deterministic rules from diagnostic_signal / tool errors; fall back
        to a short LLM call only when the root cause is unclear.
        """
        signal = diagnostic_signal or getattr(verification, "diagnostic_signal", None) or {}
        hypo = signal.get("causal_hypothesis") or {}
        target = str(
            hypo.get("target_variable")
        ...
        reason = str(
            ...
            or hypo.get("description")
            or signal.get("analysis")
            or getattr(verification, "analysis", "")
            or "subgoal incomplete"
        )
        tool_name = getattr(step_ctx, "tool_name", "") or ""
        result = getattr(step_ctx, "result_executor", None)
        result_text = str(result or "").lower()

        attributed = "actor"
        recovery = "Retry with different parameters or a sharper query."
        if (
            "timeout" in result_text
            or "connection" in result_text
            or "403" in result_text
            or "404" in result_text
            or "pdf" in result_text and "fail" in result_text
            or target in {"tool", "external", "environment"}
            or signal.get("tool_appropriate") is False
            or getattr(verification, "tool_appropriate", True) is False
        ):
            attributed = "tool_environment"
            recovery = "Switch tool or repair the environment/source constraint."
        elif target in {"planner_belief", "task_graph"} or signal.get("failure_type") in {
            "WrongSubgoal",
            "PlanError",
        }:
            attributed = "planner"
            recovery = "Revise belief or decompose the remaining goal."
        elif target in {"executor", "query", "command", "coverage"}:
            attributed = "actor"
            recovery = "Retry with different parameters on the same tool."
        elif str(signal.get("root_cause_confidence") or hypo.get("confidence") or "").upper() == "LOW":
            attributed = "verifier"
            recovery = "Re-verify with a tighter evidence contract before escalating."

        # Optional LLM refinement when still unclear / LOW confidence.
        conf = str(hypo.get("confidence") or signal.get("root_cause_confidence") or "MEDIUM").upper()
        if conf == "LOW" and getattr(self, "llm_engine", None) is not None:
            try:
                prompt = (
                    "Attribute this agent failure to exactly one of: "
                    "planner, actor, verifier, tool_environment.\n"
                    f"Question: {question}\n"
                    f"Subgoal: {getattr(step_ctx, 'sub_goal', '')}\n"
                    f"Tool: {tool_name}\n"
                    f"Analysis: {getattr(verification, 'analysis', '')}\n"
                    f"Diagnostic: {json.dumps(signal, ensure_ascii=False)[:1200]}\n"
                    "Return attributed_to, reason, recovery_hint."
                )
                ...
                if isinstance(response, FailureAttributionOutput):
                    return response
                ...
                if isinstance(response, FailureAttributionOutput):
                    return response
                if isinstance(response, dict):
                    return FailureAttributionOutput(
                        attributed_to=str(response.get("attributed_to", attributed)),
                        reason=str(response.get("reason", reason))[:400],
                        recovery_hint=str(response.get("recovery_hint", recovery))[:400],
                    )
            except Exception:
                pass
    ...
    def audit_final_answer(
        ...
        *,
        ...
        compact_view: Optional[Dict[str, Any]] = None,
    ) -> FinalAuditDecision:
        """Lightweight FinalAudit: answer must be grounded in live verified facts."""
        facts = valid_facts or {}
        if self.system_memory and not facts:
            facts = getattr(self.system_memory, "current_state_valid_facts", lambda: {})()
        view = compact_view or {}
        if self.system_memory and not view:
            getter = getattr(self.system_memory, "compact_task_view", None)
            if callable(getter):
                view = getter()
        ...
        if not answer_text:
            return FinalAuditDecision(
                ...
                unsupported_claims=["<empty>"],
            )

        # Deterministic grounding: if we have slot-aligned facts, require overlap.
        unsupported: List[str] = []
        if facts:
            joined_facts = " ".join(str(v) for v in facts.values()).lower()
            # Check answer tokens of length >= 3 against fact blob.
            tokens = [
                t for t in re.findall(r"[A-Za-z0-9][A-Za-z0-9\-\./%]{2,}", answer_text)
                if t.lower() not in {"the", "and", "for", "with", "from", "that", "this"}
            ]
            grounded = False
            for tok in tokens[:12]:
                if tok.lower() in joined_facts:
                    grounded = True
                    break
            # Numeric / short answers: direct containment either way.
            if answer_text.lower() in joined_facts or any(
                str(v).lower() in answer_text.lower() for v in facts.values() if str(v)
            ):
                grounded = True
            if not grounded and tokens:
                unsupported.append(answer_text[:200])

        missing = list((view or {}).get("missing_slots") or [])
        if missing:
            return FinalAuditDecision(
                verdict="FAIL",
                reason=f"Missing required slots: {missing}",
                unsupported_claims=unsupported or missing,
            )
        if unsupported:
            return FinalAuditDecision(
                verdict="FAIL",
                reason="Final answer is not grounded in live verified facts.",
                unsupported_claims=unsupported,
            )
        return FinalAuditDecision(
            verdict="PASS",
            reason="Answer grounded in live verified facts.",
            unsupported_claims=[],
        )
