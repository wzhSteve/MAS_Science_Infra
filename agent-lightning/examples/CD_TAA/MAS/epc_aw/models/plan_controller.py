...
class PlanControllerMixin:
    ...
    def can_attempt_done(self, question: str, analysis: str = "") -> bool:
        """StructAgent-aligned DONE predicate over verified facts + causal graph."""
        return self._can_attempt_done(question, analysis)

    def _struct_stop_enabled(self) -> bool:
        mem = getattr(self, "system_memory", None)
        if mem is None:
            return False
        ablation = getattr(self, "ablation", None)
        if ablation is not None:
            return bool(getattr(ablation, "enable_struct_stop_gate", True))
        return bool(getattr(mem, "enable_struct_stop_gate", True))

    def _slot_stop_inputs(self):
        mem = self.system_memory
        require_live = self._struct_stop_enabled()
        getter = getattr(mem, "slot_evidence_records", None)
        if callable(getter):
            records = getter(require_live_verified=require_live)
        else:
            records = mem.evidence_records
        valid_facts = getattr(mem, "current_state_valid_facts", lambda: {})()
        return records, valid_facts, require_live

    def _graph_has_open_acquisition(self) -> bool:
        mem = self.system_memory
        graph = getattr(mem, "causal_graph", None)
        if graph is None:
            return False
        valid_facts = getattr(mem, "current_state_valid_facts", lambda: {})()
        open_fn = getattr(graph, "open_acquisition_subgoals", None)
        if not callable(open_fn):
            return False
        # Only enforce when at least one subgoal carries an explicit contract,
        # otherwise legacy tasks with contract-less subgoals would never STOP.
        open_items = open_fn(valid_facts)
        contracted = [
            item for item in open_items if item.get("required_fact_names")
        ]
        return bool(contracted)

    def _graph_has_any_subgoal_contract(self) -> bool:
        graph = getattr(self.system_memory, "causal_graph", None)
        if graph is None:
            return False
        for node in getattr(graph, "nodes", {}).values():
            if node.get("type") == "Subgoal" and node.get("required_fact_names"):
                return True
        return False

    def _can_attempt_done(self, question: str, analysis: str = "") -> bool:
        profile = self.system_memory.get_task_profile()
        records, valid_facts, require_live = self._slot_stop_inputs()
        if profile:
            if not SlotGate.can_stop(
                profile,
                records,
            ...
            if SlotGate.has_compute_slot(profile):
                if not SlotGate.compute_slot_filled_by_computed(profile, records):
                    return False
            elif SlotGate.needs_compute_step(profile, records):
                return False
        else:
            cumulative = self._cumulative_evidence_text()
            if not self.diagnoser._answer_verified_ready(question, cumulative, analysis):
                return False

        if self._struct_stop_enabled() and self._graph_has_open_acquisition():
            return False

        # Outline is a compatibility projection. When Subgoal contracts exist,
        # the graph is authoritative; otherwise keep outline acquisition veto
        # as a safety net against premature SlotGate fills.
        if self._outline_blocks_stop():
            if (
                not self._struct_stop_enabled()
                or not self._graph_has_any_subgoal_contract()
            ):
                return False
        return True
    ...
    @staticmethod
    def _mcq_ready_for_synthesis(profile: Any, records: List[Dict[str, Any]]) -> bool:
        if profile is None or profile.missing_slot_names(records) != ["final_answer"]:
            return False
        ...
        return (
            is_multiple_choice
            and SlotGate.can_enter_synthesize(profile, records)
        )

    @staticmethod
    def _mcq_synthesis_attempted(records: List[Dict[str, Any]]) -> bool:
        return any(
            str(record.get("tool", "")) == "Base_Generator_Tool"
    ...
    def _ensure_outline_nonempty(
        ...
        records = self.system_memory.evidence_records
        if (
            self._mcq_ready_for_synthesis(profile, records)
            and self._mcq_synthesis_attempted(records)
        ):
            return {}
    ...
    def _can_stop_execution(self, question: str, analysis: str = "") -> bool:
        """Hard gate: STOP via can_attempt_done (verified slots + optional graph)."""
        if self._struct_stop_enabled():
            return self._can_attempt_done(question, analysis)

        # Legacy path (no_struct_control): SlotGate on raw evidence + outline block.
        profile = self.system_memory.get_task_profile()
        records = self.system_memory.evidence_records
        if profile:
            if not SlotGate.can_stop(
                profile, records, require_live_verified=False,
            ):
                return False
            if self._outline_blocks_stop():
                return False
            ...
            if SlotGate.needs_compute_step(profile, records):
                outline = self.system_memory.get_outline() or {}
                if any("Python_Coder_Tool" in str(v) for v in outline.values()):
                    return False
            return True
    ...
    def _audit_and_maybe_invalidate_final(
        self,
        question: str,
        answer: str,
        *,
        ...
        """Run FinalAudit; on FAIL invalidate responsible evidence. Returns PASS?"""
        ablation = getattr(self, "ablation", None)
        enabled = True
        if ablation is not None:
            enabled = bool(getattr(ablation, "enable_final_audit", True))
        else:
            enabled = bool(
                getattr(self.system_memory, "enable_final_audit", True)
            )
        if not enabled:
            return True

        audit_fn = getattr(self.diagnoser, "audit_final_answer", None)
        ...
        decision = audit_fn(
            ...
            compact_view=self.system_memory.compact_task_view(),
        )
        verdict = getattr(decision, "verdict", None) or (
            decision.get("verdict") if isinstance(decision, dict) else "PASS"
        )
        if str(verdict).upper() == "PASS":
            return True

        reason = getattr(decision, "reason", None) or (
            decision.get("reason") if isinstance(decision, dict) else "audit failed"
        )
        # Invalidate the most recent live verified evidence as responsibility rollback.
        from MAS.epc_aw.models.verified_memory import CommitKind, MemoryCommitEvent

        live = self.system_memory.live_evidence_records()
        evidence_id = live[-1]["id"] if live else None
        ...
        if callable(commit):
            commit(
                MemoryCommitEvent(
                    ...
                    recovery_hint="Collect stronger verified evidence before answering.",
                )
            )
        print(f"\n==> 🔎 FinalAudit FAIL — invalidated evidence ({reason})\n")
        return False
    ...
    def _generate_recovery_outline(
        ...
        if self._mcq_ready_for_synthesis(profile, records):
            if self._mcq_synthesis_attempted(records):
                return {}
    ...
    def _sanitize_outline_update(
        ...
        if (
            result
            and self._mcq_ready_for_synthesis(profile, records)
            and self._mcq_synthesis_attempted(records)
        ):
            result_text = " ".join(str(value).lower() for value in result.values())
            if any(marker in result_text for marker in (
                "base_generator", "final answer", "synthesize",
            )):
                result = {}
        ...
        profile = self.system_memory.get_task_profile()
        if profile and not self._mcq_ready_for_synthesis(
            profile, self.system_memory.evidence_records,
        ) and infer_subgoal_kind(
