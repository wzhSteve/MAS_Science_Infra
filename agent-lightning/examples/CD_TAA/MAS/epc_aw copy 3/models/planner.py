...
class Planner:
    ...
    def _self_criticize_analysis(
        ...
        is_quantity_question = bool(re.search(r"\bhow many\b|\bhow much\b|\bhow many\b", q))
        ...
        # Skip the extra LLM call when the outline already retrieves external
        # evidence and the analysis does not foreclose on a fixed answer.
        ...
        # Extract the JSON object following the marker line (case-insensitive
        # marker match; preserve original case of the JSON payload that follows).
        json_text = resp[marker_idx + len(marker):].strip()
        try:
            parsed_rev = parse_json_from_llm_response(json_text)
        except Exception:
            return analysis, execution_outline
        rev_analysis = parsed_rev.get("Analysis", "")
        rev_outline = parsed_rev.get("ExecutionOutline", {})
        if rev_outline and isinstance(rev_outline, dict):
            print(
                "\n==> 🔄 Step 0 self-critique: detected foreclosing misread; "
                "revised outline to retrieve external base quantity.\n"
            )
