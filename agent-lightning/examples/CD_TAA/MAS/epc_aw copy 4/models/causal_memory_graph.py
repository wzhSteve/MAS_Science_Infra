...
class CausalMemoryGraph:
    ...
    def link_evidence_to_outcome(self, outcome_id: str, evidence_id: str) -> None:
        ev_node = self._evidence_node_id(evidence_id)
        if ev_node not in self.nodes:
            self.add_node(ev_node, GraphNodeType.EVIDENCE, evidence_id=evidence_id)
        self.add_edge(outcome_id, ev_node, ExecutionEdgeType.OUTCOME_EXTRACTS_EVIDENCE)
        if self.current_state_id:
            self.add_edge(ev_node, self.current_state_id, ExecutionEdgeType.EVIDENCE_CONTRIBUTES_STATE)
