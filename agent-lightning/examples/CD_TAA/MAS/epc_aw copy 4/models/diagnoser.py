...
class Diagnoser:
    ...
    def verificate_context(self, question: str, image: str, target_information: str, outline: Dict[str, Any], result_executor: str, step_count: int = 0, obtained_information: Any = None, sub_goal: str = "", tool_name: str = "", intervention_context: Optional[Dict[str, Any]] = None, current_outline_step: Optional[str] = None) -> Any:
        ...
        if step_conclusion == "SUBGOAL_COMPLETE" and character_slot_required:
            ...
            canonical_name = _extract_character_name(
                " ".join(candidate_values) + " " + result_text
            )
            if canonical_name and grounded:
                ...
                not grounded
                or not any(_is_character_name_answer(value) for value in candidate_values)
            ):
                step_conclusion = "SUBGOAL_INCOMPLETE"
                Conclusion = "CONTINUE"
                slot_updates = [
    ...
    _REASONS_BY_TARGET = {
        ...
        "command":         {"schema_mismatch", "missing_argument"},
    ...
    def _classify_root_cause(
        ...
        # N7: Retrieval failure (ABSENCE / EMPTY). Sub-classify by query
        # content: does the sub_goal carry an entity anchor? If it does
        # and we still got nothing, the failure is more likely CoverageLow
        # than a vague query. MEDIUM confidence — ABSENCE classification
        # is rule-based, not marker-based.
