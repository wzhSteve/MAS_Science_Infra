...
class ToolPolicyMixin:
    ...
    def _resolve_tool_for_step(
        ...
        if rec == "switch_tool" or rec == "revise_belief":
            ...
            # Synthesis must not be overridden onto retrieval tools (L2b already
            # filters this; hot-path Tool override must match).
            if kind == "synthesis" and suggested in _search:
                suggested = None
            if (
                kind != "compute"
                and suggested
