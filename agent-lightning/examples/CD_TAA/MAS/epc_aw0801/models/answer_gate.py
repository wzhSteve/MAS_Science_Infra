...
class AnswerGateMixin:
    ...
    def _answer_format_mismatch(question: str, answer: str) -> Optional[str]:
        ...
        if not ans:
            return "empty answer"
        if _extract_lettered_options(question):
            if _canonicalize_multiple_choice_answer(question, ans):
                return None
            return "multiple-choice answer does not map uniquely to a listed option"
        ...
        # "who was/is" / US president → answer should look like a person name
        if re.search(r"\bwho\s+(was|is|were|are)\b", q) or re.search(
            r"\bpresident of the united states\b|\bwhich\s+president\b", q,
        ):
            if re.search(r"^\d{3,4}$", ans):
                return "question asks who/president but answer is a bare year"
            if re.search(r"\b(was\s+founded|founded\s+in|founding\s+year)\b", ans, re.I):
                return "question asks who/president but answer is a founding-year factoid"
            if not SlotGate._looks_like_person_name_answer(ans):
                return "question asks who/president but answer does not look like a person name"
    ...
    def _normalize_final_answer(
        ...
        if not raw:
            return raw
        canonical_choice = _canonicalize_multiple_choice_answer(
            question, raw, evidence_hint,
        )
        if canonical_choice:
            return canonical_choice
        tag_m = re.search(r"<answer>\s*(.*?)\s*</answer>", raw, re.I | re.S)
        if tag_m:
        ...
        if "{" in raw or ('"' in raw and "character" in raw.lower()):
            ...
            if name:
                return name
        force_llm = SlotGate._is_where_place_question(question)
        # Short answers skip LLM unless where/place needs modern-form resolution.
        if (
            not force_llm
            and len(raw) <= 30
            and len(raw.split()) <= 4
        ...
        if force_llm:
            place_rule = (
                "Prefer the modern common-form place/country the question expects "
                "when both historical and modern forms appear "
                "(e.g. Oranienburg, Germany).\n"
            )
        prompt = (
            ...
            "for (a number, a place name, a person name, or a short term). No explanation.\n"
            f"{place_rule}\n"
            f"Question: {question}\n"
        ...
        try:
            ...
            # Unwrap accidental <answer> tags from the extractor.
            tag2 = re.search(r"<answer>\s*(.*?)\s*</answer>", extracted, re.I | re.S)
            if tag2:
                extracted = tag2.group(1).strip()
            if not extracted:
                return raw
            if force_llm or len(extracted) <= len(raw):
                return extracted
