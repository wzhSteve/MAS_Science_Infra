...
class SystemMemory(Memory):
    ...
    def __init__(self, toolbox_metadata: Dict[str, Any], agent_profile: Dict[str, Any] = None):
        ...
        self.online_memory = {
            ...
            # 结构: [{factor: {state, subgoal}, effects: [...], L2_diagnosis}]
    ...
    def record_causal_trace(
        ...
        Args:
            ...
            state: 当前信息状态
            subgoal: 目标需求
            effects: 多个尝试 [{attempt_seq, tool, parameter, result, symptom, L1_diagnosis}]
            L2_diagnosis: 根因分析 {root_cause, recommendation}
        """
        trace = {
            "step": step,
            "factor": {
    ...
    def add_failed_parameter(
        ...
        """在任务本地参数中添加失败参数"""
        params = self.get_task_local_parameters(state, subgoal, tool)
        failed_list = params.get("failed_parameters", [])
    ...
    def add_successful_parameter(
        ...
        """在任务本地参数中添加成功参数"""
        params = self.get_task_local_parameters(state, subgoal, tool)
        successful_list = params.get("successful_parameters", [])
