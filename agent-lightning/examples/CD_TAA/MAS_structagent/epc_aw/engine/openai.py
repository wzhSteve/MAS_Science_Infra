...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.model_string = (
            os.getenv("MODEL_NAME") or os.getenv("MODEL_Name") or model_string
        )
    ...
    def _generate_text(
        ...
        # Chat models without structured outputs
            ...
            response = self.client.chat.completions.create(
                ...
                top_p=top_p,
                n=n,
                # Qwen thinking mode only supports n=1. Disable it for server
                # models so multiple completions remain available.
                extra_body=(
                    {"enable_thinking": False}
                    if self.model_string in self.SERVER_MODEL
                    else None
                ),
            )
            ...
            if n == 1:
                response = _normalize_content(response.choices[0].message.content)
            else:
                response = [
                    _normalize_content(choice.message.content)
